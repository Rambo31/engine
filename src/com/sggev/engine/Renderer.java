...
import com.sggev.engine.gfx.Font;
import com.sggev.engine.gfx.Image;
import com.sggev.engine.gfx.ImageRequest;
import com.sggev.engine.gfx.ImageTile;
import com.sggev.engine.gfx.Light;
import com.sggev.engine.gfx.LightRequest;
...
public class Renderer {
	...
	public void drawReverseImageTile(ImageTile image, int offX, int offY, int tileX, int tileY)
	{
		offX -= camX;
		offY -= camY;
		...
		for(int y = newY; y < newHeight; y++)
		{
			for(int x = newX; x < newWidth; x++)
			{
				setPixel(-x + offX + newWidth, y + offY, image.getP()[(x + tileX * image.getTileW()) + (y + tileY * image.getTileH()) * image.getW()]);
				setLightBlock(x + offX, y + offY, image.getLightBlock());
			}
		}
	}	
	...
	public void drawFillCircle(int offX, int offY, int radius, int color)
	{
		...
		for(int y = -radius; y <= radius; y++)
		{
			...
			{
					if(((x * x) + (y * y)) <= (radius * radius)) setPixel(x + offX, y + offY, color);
			}	
		}
	}
